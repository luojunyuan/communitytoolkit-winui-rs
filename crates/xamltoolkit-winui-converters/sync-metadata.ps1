...
function Get-ProjectPackagePath([string]$PackagesRoot, [string]$ProjectRoot, [string]$PackageId) {
    ...
    $projectPath = Join-Path $ProjectRoot "XamlToolkit.WinUI.Converters.vcxproj"
...
function Get-ConvertersOutput([string]$SourceRoot, [string]$Platform, [string]$Configuration) {
    $candidates = @(
        (Join-Path $SourceRoot "$Platform\$Configuration\XamlToolkit.WinUI.Converters")
    )

    if ($Platform -eq "Win32") {
        $candidates += (Join-Path $SourceRoot "$Configuration\XamlToolkit.WinUI.Converters")
    }

    return Get-ExistingPath $candidates "XamlToolkit.WinUI.Converters $Platform|$Configuration build output"
}

function Copy-NativeRuntime([string]$ConvertersOutput, [string]$Destination) {
    New-Item -ItemType Directory -Force $Destination | Out-Null

    foreach ($name in @("XamlToolkit.WinUI.Converters.dll", "XamlToolkit.WinUI.Converters.pri", "XamlToolkit.WinUI.Converters.winmd")) {
        $source = Join-Path $ConvertersOutput $name
...
$convertersOutput = Get-ConvertersOutput $SourceRoot $Platform $Configuration
$convertersWinmd = Join-Path $convertersOutput "XamlToolkit.WinUI.Converters.winmd"
$projectRoot = Join-Path $SourceRoot "XamlToolkit.WinUI.Converters"
$projectPath = Join-Path $projectRoot "XamlToolkit.WinUI.Converters.vcxproj"
...
if (!(Test-Path -LiteralPath $convertersWinmd)) {
    throw "Missing $convertersWinmd. Build XamlToolkit.WinUI.Converters.vcxproj for $Platform|$Configuration first."
}
...
Copy-Item -LiteralPath $convertersWinmd -Destination (Join-Path $metadataDir "XamlToolkit.WinUI.Converters.winmd") -Force
foreach ($dep in $deps) {
    Copy-Item -LiteralPath $dep -Destination (Join-Path $depsDir (Split-Path -Leaf $dep)) -Force
}

$rootWindowsWinmd = Join-Path $workspaceRoot "crates\xamltoolkit-winui\metadata\deps\Windows.winmd"
$localWindowsWinmd = Join-Path $depsDir "Windows.winmd"
if ((Test-Path -LiteralPath $rootWindowsWinmd) -and !(Test-Path -LiteralPath $localWindowsWinmd)) {
    Copy-Item -LiteralPath $rootWindowsWinmd -Destination $localWindowsWinmd -Force
}
...
Copy-NativeRuntime $convertersOutput (Join-Path $nativeDir $Platform)

Write-Host "Synced XamlToolkit.WinUI.Converters metadata from $convertersOutput"
